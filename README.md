...
- Questions: `shared/questions.ts`. Keep these ids, which the code reads: investor `investorName`, `fundName`, `stages`, `tickets`, `thesis`, `regions`, `founderFit`; founder `companyName`, `values`, `stage`, `raise`, `problem`, `solution`, `traction`, `team`, `whyInvest`; both `involvement`, `pressure`, `transparency` and `leadership`. `involvement` option ids must be taxonomy ids. Neither role has a website question; founders pick their company's three main values (`values`, a multiple-choice list of values with at most 3 picks) instead of a one-liner. The founder `raise` and investor `tickets` questions are both dual-handle range sliders sharing `TICKET_STOPS` (also in `shared/questions.ts`), each answered as a `[min, max]` pair of stop values.
